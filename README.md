# Estúdio de Ideias

Sistema web de gestão do conhecimento acadêmico desenvolvido para o curso de Tecnologia em Análise e Desenvolvimento de Sistemas da PUC Minas Virtual.

O **Estúdio de Ideias** funciona como um repositório digital de projetos acadêmicos, permitindo o armazenamento, organização e consulta de trabalhos desenvolvidos por estudantes ao longo da graduação. Inspirado em plataformas como o Google Scholar, o sistema busca preservar o conhecimento produzido na universidade e facilitar seu acesso por estudantes, pesquisadores e educadores.

## Funcionalidades

* Pesquisa de projetos por palavras-chave;
* Filtros e categorização por áreas de conhecimento;
* Sistema de favoritos;
* Visualização de estatísticas da plataforma;
* Submissão de projetos para moderação;
* Consulta e compartilhamento de trabalhos publicados.

## Tecnologias Utilizadas

* HTML5
* CSS3
* JavaScript
* Bootstrap

## Documentação

1. [Documentação de Contexto](documentos/01-Documentação%20de%20Contexto.md)
2. [Especificação do Projeto](documentos/02-Especificação%20do%20Projeto.md)
3. [Metodologia](documentos/03-Metodologia.md)
4. [Projeto de Interface](documentos/04-Projeto%20de%20Interface.md)
5. [Template Padrão da Aplicação](documentos/05-Template%20padrão%20da%20Aplicação.md)
6. [Programação de Funcionalidades](documentos/06-Programação%20de%20Funcionalidades.md)
7. [Plano de Testes de Software](documentos/07-Plano%20de%20Testes%20de%20Software.md)
8. [Registro de Testes de Software](documentos/08-Registro%20de%20Testes%20de%20Software.md)
9. [Referências Bibliográficas](documentos/09-Referências%20Bibliográficas.md)

## Demonstração

🔗 Acesse o projeto:
https://icei-puc-minas-pmv-ads.github.io/pmv-ads-2025-1-e1-proj-web-t5-projestudiodeideias-1/codigo-fonte/index.html

## Código-Fonte

* [Código Fonte](codigo-fonte/README.md)
